...
export async function sendSelectionEmail(
  ...
  const subject = `Congratulations! You've been selected — ${role}`;
  const html = wrapHtml(`
    <h1 style="margin:0 0 16px;font-size:24px;font-weight:700;color:#111827">Great News!</h1>
    <p style="color:#4b5563;font-size:15px;line-height:1.6;margin:0 0 20px">
      Hi ${escapeHtml(candidateName || "there")}, we're pleased to inform you that you have been <strong style="color:#059669">selected</strong> to move forward in the hiring process for the <strong style="color:#111827">${escapeHtml(role)}</strong> role.
    </p>
    <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:16px;margin:0 0 24px">
      <p style="color:#166534;font-size:14px;margin:0;line-height:1.6">
        Our hiring team will reach out to you shortly with the next steps. Please keep an eye on your inbox.
      </p>
    </div>
    <p style="color:#6b7280;font-size:14px;line-height:1.6;margin:0">
      Thank you for your time and interest in joining ${escapeHtml(orgName)}. We look forward to speaking with you.
    </p>
  `, orgName);
  await sendMail(to, subject, html);
}
...
export async function sendRejectionEmail(
  ...
  const html = wrapHtml(`
    <h1 style="margin:0 0 16px;font-size:24px;font-weight:700;color:#111827">Application Update</h1>
    <p style="color:#4b5563;font-size:15px;line-height:1.6;margin:0 0 20px">
      Hi ${escapeHtml(candidateName || "there")}, thank you for taking the time to interview for the <strong style="color:#111827">${escapeHtml(role)}</strong> role at ${escapeHtml(orgName)}.
    </p>
    <p style="color:#4b5563;font-size:15px;line-height:1.6;margin:0 0 20px">
      After careful consideration, we have decided to move forward with other candidates whose experience more closely matches our current requirements. This was a competitive process, and we appreciate the effort you put into the interview.
    </p>
    <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;padding:16px;margin:0 0 24px">
      <p style="color:#6b7280;font-size:14px;margin:0;line-height:1.6">
        We encourage you to apply for future openings that match your skills. We will keep your profile on file for upcoming opportunities.
      </p>
    </div>
    <p style="color:#6b7280;font-size:14px;line-height:1.6;margin:0">
