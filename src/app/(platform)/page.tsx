...
// 40% ATS (0-100) + 60% interview (1-5 → 0-100). Returns 0-100 or null.
function compositeScore(atsScore: number | null | undefined, overall: number | null | undefined): number | null {
  const hasAts = atsScore != null;
  const hasIv = overall != null;
  if (hasAts && hasIv) return Math.round(0.4 * atsScore! + 0.6 * (overall! / 5 * 100));
  if (hasAts) return Math.round(atsScore!);
  if (hasIv) return Math.round(overall! / 5 * 100);
  return null;
}
...
function compositeStroke(score: number) {
  ...
  const filtered = useMemo(() => {
    ...
    result = [...result].sort((a, b) => {
      ...
      if (sortField === "date") {
        ...
        cmp = (compositeScore((a as any).atsScore, a.scorecard?.overall) ?? -1) - (compositeScore((b as any).atsScore, b.scorecard?.overall) ?? -1);
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4 text-xs text-gray-500">
                      ...
                      {(() => {
                        const comp = compositeScore((interview as any).atsScore, interview.scorecard?.overall);
                        return comp !== null ? (
                          <span className={`font-medium ${compositeColor(comp)}`}>{comp}/100</span>
                        ) : null;
                      })()}
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            {(() => {
                              const comp = compositeScore((interview as any).atsScore, interview.scorecard?.overall);
                              if (comp === null) return <span className="text-sm text-gray-400">-</span>;
                              ...
                              return (
                                <div className="space-y-1.5">
                                    ...
                                    <div className="relative w-8 h-8">
                                      <svg className="w-8 h-8 -rotate-90" viewBox="0 0 36 36">
                                        ...
                                        <circle cx="18" cy="18" r="14" fill="none"
                                          stroke={compositeStroke(comp)}
                                          strokeWidth="3" strokeLinecap="round"
                                          strokeDasharray={`${(comp / 100) * circ} ${circ}`}
                                        />
                                      </svg>
                                      <span className={`absolute inset-0 flex items-center justify-center text-[9px] font-bold ${compositeColor(comp)}`}>
                                        {comp}
                                      </span>
                                    </div>
                                  ...
                                  <div className="flex gap-1">
                                    {(interview as any).atsScore != null && (
                                      <span className="text-[9px] px-1.5 py-0.5 bg-indigo-50 text-indigo-600 rounded font-medium">
                                        ATS {Math.round((interview as any).atsScore)}
                                      </span>
                                    )}
                                    {interview.scorecard?.overall != null && (
                                      <span className="text-[9px] px-1.5 py-0.5 bg-gray-100 text-gray-600 rounded font-medium">
                                        IV {interview.scorecard.overall}/5
                                      </span>
                                    )}
                                  </div>
                                </div>
