        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-lg shadow-gray-200/50 p-8 sm:p-10 text-center animate-fade-in-up">
          ...
          <div className="bg-gray-50 rounded-xl border border-gray-100 p-5 mb-8 text-left space-y-4 animate-fade-in-up" style={{ animationDelay: "500ms", opacity: 0 }}>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-gray-500 font-medium uppercase tracking-wider">Interview Duration</p>
                <p className="text-sm text-gray-800 font-medium">{interview?.duration || 30} minutes allocated</p>
                {interview?.startedAt && interview?.endedAt && (() => {
                  const actualMin = Math.round((new Date(interview.endedAt).getTime() - new Date(interview.startedAt).getTime()) / 60000);
                  return <p className="text-xs text-gray-500 mt-0.5">Actual time: {actualMin} min</p>;
                })()}
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {interview?.endedAt && (
                  <p className="text-xs text-gray-400 mt-0.5">Completed {new Date(interview.endedAt).toLocaleString()}</p>
                )}
              </div>
            </div>
          </div>
