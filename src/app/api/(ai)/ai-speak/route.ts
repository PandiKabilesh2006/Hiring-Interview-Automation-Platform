...
export async function POST(req: Request) {
  try {
    ...
    if (!isEnglish && lastEntry) {
      // Save candidate's non-English message
      if (!skipSave) {
      ...
      try {
        ...
        const [audioBuffer] = await Promise.all([
          ttsProvider.synthesize(warningText),
        ...
        console.warn("Warning TTS failed:", (err as Error).message);
        return NextResponse.json({ text: warningText, audio: null, contentType: null });
      }
    }
