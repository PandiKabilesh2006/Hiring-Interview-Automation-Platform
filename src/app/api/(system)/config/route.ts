...
export async function GET() {
  return NextResponse.json({
    // Proctoring
    maxProctoringStrikes: parseInt(process.env.MAX_PROCTORING_STRIKES || process.env.NEXT_PUBLIC_MAX_PROCTORING_STRIKES || "5"),

    // STT
    sttProviders: (process.env.STT_CLIENT_PROVIDERS || "soniox,browser").split(",").map(s => s.trim()),
