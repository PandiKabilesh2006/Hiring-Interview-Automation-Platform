...
import { ENGLISH_ONLY_WARNING, NON_ENGLISH_TRANSCRIPT_TEXT, displayCandidateSpeech, sanitizeCandidateSpeech } from "@/lib/language";
...
function normalizeTranscriptEntries(entries: any[] = []): TranscriptEntry[] {
  return entries
    ...
    .filter((t) => (t.role === "ai" || t.role === "candidate") && t.text);
}
...
export function InterviewRoom({ interviewId }: { interviewId: string }) {
  ...
  const [runtimeConfig, setRuntimeConfig] = useState({
    maxProctoringStrikes: 5,
    sttProviders: ["soniox", "browser"] as ("soniox" | "browser")[],
    sttBackend: "soniox",
    silenceDelayMs: 4000,
  });
  const maxProctoringStrikes = Math.max(1, runtimeConfig.maxProctoringStrikes || 5);
  const proctoringDangerThreshold = Math.max(1, maxProctoringStrikes - 2);
  const proctoringFinalWarningThreshold = Math.max(1, maxProctoringStrikes - 1);
  useEffect(() => {
  ...
  const tokenRef = useRef(typeof window !== "undefined" ? new URLSearchParams(window.location.search).get("token") || "" : "");
      ...
      try {
        ...
        if (hasStarted && interview.status !== "completed") {
          ...
          if (interview.transcript?.length > 0) {
            setTranscript(normalizeTranscriptEntries(interview.transcript));
          }
          ...
          try {
            ...
            const maxStrikes = cfgRes?.maxProctoringStrikes || 5;
            if (violationRes.ok) {
              const { count } = await violationRes.json();
              setProctoringWarnings(Math.min(count, maxStrikes));
  ...
  useEffect(() => {
    ...
    async function setupMedia() {
      try {
        ...
        if (!selectedCameraId && activeCameraId) setSelectedCameraId(activeCameraId);
        if (!selectedMicId && activeMicId) setSelectedMicId(activeMicId);
  ...
  }, [consentGiven, selectedCameraId, selectedMicId]);
  ...
  // Keep candidate-side transcript aligned with persisted server entries.
  // Local STT still appears immediately; polling fills in entries saved by API calls
  // or another resumed tab/session.
  useEffect(() => {
    if (!isStarted || isEndingRef.current) return;
    const tokenParam = tokenRef.current ? `?token=${tokenRef.current}` : "";

    const mergeServerTranscript = async () => {
      try {
        const res = await fetch(`/api/interview/${interviewId}${tokenParam}`);
        if (!res.ok) return;
        const data = await res.json();
        const serverTranscript = normalizeTranscriptEntries(data.transcript);
        if (serverTranscript.length === 0) return;

        setTranscript((prev) => {
          const seen = new Set(prev.map((entry) => `${entry.role}\u0000${entry.text}`));
          const additions = serverTranscript.filter((entry) => !seen.has(`${entry.role}\u0000${entry.text}`));
          return additions.length ? [...prev, ...additions].sort((a, b) => a.timestamp - b.timestamp) : prev;
        });
      } catch {}
    };

    mergeServerTranscript();
    const interval = setInterval(mergeServerTranscript, 3000);
    return () => clearInterval(interval);
  }, [isStarted, interviewId]);
      ...
      try {
        ...
        if (!res.ok || !res.body) {
          ...
          if (data.text) {
            setTranscript((prev) => {
              const updated = [...prev];
              if (data.text === ENGLISH_ONLY_WARNING) {
                for (let i = updated.length - 1; i >= 0; i--) {
                  if (updated[i].role === "candidate") {
                    updated[i] = { ...updated[i], text: NON_ENGLISH_TRANSCRIPT_TEXT };
                    break;
                  }
                }
              }
              updated.push({ role: "ai", text: data.text, timestamp: Date.now() });
              return updated;
            });
            if (data.audio) {
              const audioBytes = Uint8Array.from(atob(data.audio), (c) => c.charCodeAt(0));
            ...
            try {
              ...
              if (data.type === "text") {
                ...
                if (!transcriptAdded) {
                  ...
                  setTranscript((prev) => {
                    const updated = [...prev];
                    if (data.text.startsWith(ENGLISH_ONLY_WARNING)) {
                      for (let i = updated.length - 1; i >= 0; i--) {
                        if (updated[i].role === "candidate") {
                          updated[i] = { ...updated[i], text: NON_ENGLISH_TRANSCRIPT_TEXT };
                          break;
                  ...
                  setTranscript((prev) => {
                    const u = [...prev];
                    if (u.length > 0 && u[u.length-1].role === "ai") u[u.length-1] = { ...u[u.length-1], text: fullText };
                    return u;
                  });
                }
              }
              ...
              if (data.type === "done") {
                if (data.fullText) {
                  setTranscript((prev) => {
                    const u = [...prev];
                    if (data.fullText === ENGLISH_ONLY_WARNING) {
                      for (let i = u.length - 1; i >= 0; i--) {
                        if (u[i].role === "candidate") {
                          u[i] = { ...u[i], text: NON_ENGLISH_TRANSCRIPT_TEXT };
                          break;
                        }
                      }
                    }
                    if (u.length > 0 && u[u.length-1].role === "ai") u[u.length-1] = { ...u[u.length-1], text: data.fullText };
                    return u;
                  });
                }
  ...
  const sendChatMessage = useCallback(async () => {
    ...
    const displayedText = displayCandidateSpeech(text);
    const pipelineText = displayedText === NON_ENGLISH_TRANSCRIPT_TEXT ? text : sanitizeCandidateSpeech(text);

    const entry: TranscriptEntry = { role: "candidate", text: displayedText, timestamp: Date.now() };
    setTranscript((prev) => {
      const updated = [...prev, entry];
      getAIResponse(updated.map((item, idx) => idx === updated.length - 1 ? { ...item, text: pipelineText } : item));
      return updated;
    });
  ...
  useEffect(() => {
    ...
    const watchdog = setInterval(() => {
      ...
      if (silenceSec > 45 && !interimTranscript && !isProcessingRef.current && !isAISpeakingRef.current && !watchdogFiring) {
  ...
  const stt = useSTT({
    ...
    onInterim: (text) => {
      setInterimTranscript(displayCandidateSpeech(text));
      if (text.trim()) lastActivityRef.current = Date.now();
    },
    onComplete: async (candidateText) => {
      // Guard: drop STT results that arrive while AI is still playing audio.
      // This prevents the silence timer (which doesn't stop on AI-start) from
      // triggering a second AI call while current audio playback is ongoing.
      if (isAISpeakingRef.current) return;
      setInterimTranscript("");
      lastActivityRef.current = Date.now();

      const displayedText = displayCandidateSpeech(candidateText);
      const pipelineText = displayedText === NON_ENGLISH_TRANSCRIPT_TEXT ? candidateText : sanitizeCandidateSpeech(candidateText);
  ...
  const handleStartInterview = useCallback(async () => {
    ...
    // and connects/disconnects Soniox per turn. Just start the first turn.
  ...
  const toggleMic = useCallback(() => {
    ...
    if (audioTrack) {
      ...
      // Browser STT has its own mic handle independent of the getUserMedia track.
      // Stop it when muting so it doesn't keep transcribing silently.
  ...
  const onProctoringEvent = useCallback(
    (event: { type: string; message: string; severity?: string }) => {
      // Events that count as a warning strike (each = 1 integer warning)
      ...
      const isStrike = strikeTypes.has(event.type);
      if (isStrike) {
      ...
      if (isStrike && effectiveSeverity === "flag") {
        setProctoringWarnings((prev) => {
          const next = prev + 1;
          if (next >= maxProctoringStrikes) {
    ...
    [interviewId, maxProctoringStrikes]
  );
  ...
  // No decay — 5 integer warnings, each one counts permanently
        ...
        <div className="grid w-full max-w-[480px] gap-3 sm:grid-cols-2">
          <label className="space-y-1.5">
            ...
            <select
              ...
              className="w-full rounded-lg border border-white/10 bg-zinc-950/70 px-3 py-2 text-sm text-zinc-100 outline-none transition focus:border-blue-400 disabled:cursor-not-allowed disabled:opacity-50"
            >
                ...
                onClick={async () => {
                  try {
                    ...
                    if (displaySurface && displaySurface !== "monitor") {
                      ...
                      alert("Please share your entire screen, not a window or browser tab.");
                      return;
                    }
  ...
  return (
    <div className="flex h-screen flex-col">
      ...
      <div className={`transition-all duration-500 ${currentAIText ? "opacity-100 max-h-28" : "opacity-0 max-h-0 overflow-hidden"}`}>
        <div className="bg-gradient-to-r from-blue-900/80 via-blue-800/80 to-blue-900/80 border-b border-blue-500/30 backdrop-blur px-4 sm:px-8 py-3 flex items-start gap-3">
          <div className="mt-0.5 flex-shrink-0 flex h-6 w-6 items-center justify-center rounded-full bg-blue-500 text-white text-xs font-bold">A</div>
          <p className="text-sm sm:text-base leading-relaxed text-blue-100 line-clamp-3">{currentAIText}</p>
          {isAISpeaking && (
            <div className="ml-auto flex-shrink-0 flex items-end gap-[2px] self-center">
              {[0,1,2,3].map(i => (
                <div key={i} className="waveform w-[2px] rounded-full bg-blue-400" style={{ animationDelay: `${i * 0.15}s` }} />
              ))}
            </div>
          )}
        </div>
      ...
      {proctoringWarnings > 0 && !showProctoringBan && (
        <div className="fixed left-1/2 -translate-x-1/2 top-16 z-50">
          <div className={`flex items-center gap-3 rounded-xl px-5 py-3 shadow-lg border ${
            ...
            <span className={`text-sm font-semibold ${proctoringWarnings >= proctoringDangerThreshold ? "text-red-200" : "text-yellow-200"}`}>
              Warning {proctoringWarnings} of {maxProctoringStrikes}
              {proctoringWarnings >= proctoringFinalWarningThreshold
                ? " — One more violation will end this interview"
                : " — Please look at the screen and stay focused"}
            </span>
          </div>
        </div>
      )}
      ...
      <div className="fixed right-4 top-16 z-50 flex flex-col gap-2 max-w-xs">
        {proctoringAlerts.map((a) => {
          const isHighSeverity = ["multiple_faces", "looking_away", "camera_off", "phone_detected"].includes(a.type);
          return (
            <div
              key={a.id}
              className={`flex items-start gap-2.5 rounded-xl border px-4 py-3 shadow-lg backdrop-blur ${
                isHighSeverity
                  ? "bg-red-900/90 border-red-500/40"
                  : "bg-yellow-900/80 border-yellow-500/20"
              }`}
              style={{ animation: "fadeInRight 0.3s ease-out" }}
            >
              <svg className={`w-4 h-4 flex-shrink-0 mt-0.5 ${isHighSeverity ? "text-red-400" : "text-yellow-400"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" />
              </svg>
              <div>
                <p className={`text-xs font-bold uppercase tracking-wide ${isHighSeverity ? "text-red-300" : "text-yellow-300"}`}>
                  {a.type === "multiple_faces" ? "Another Person Detected"
                    : a.type === "looking_away" ? "Looking Away"
                    : a.type === "face_missing" ? "Face Not Visible"
                    : a.type === "camera_off" ? "Camera Disabled"
                    : a.type === "phone_detected" ? "Phone Detected"
                    : a.type === "window_blur" ? "Tab Switch / Left Window"
                    : a.type === "fullscreen_exit" ? "Exited Fullscreen"
                    : "Proctoring Alert"}
                </p>
                <p className={`text-sm mt-0.5 ${isHighSeverity ? "text-red-200" : "text-yellow-200"}`}>{a.message}</p>
              </div>
            </div>
          );
        })}
      ...
      {/* Proctoring Ban Modal — 5 warnings exceeded */}
      {showProctoringBan && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="glass w-full max-w-md rounded-2xl p-8 text-center border border-red-500/30">
            ...
            <p className="text-sm text-zinc-400 mb-4">
              You received {maxProctoringStrikes} proctoring warnings. Your interview has been ended due to repeated integrity violations:
            </p>
            <ul className="text-left text-sm text-zinc-400 space-y-1.5 mb-6 px-4">
              <li className="flex items-center gap-2"><span className="text-red-400">&#10005;</span> Looking away from the screen</li>
              <li className="flex items-center gap-2"><span className="text-red-400">&#10005;</span> Another person detected in frame</li>
              <li className="flex items-center gap-2"><span className="text-red-400">&#10005;</span> Switching tabs or leaving the interview window</li>
              <li className="flex items-center gap-2"><span className="text-red-400">&#10005;</span> Phone or device detected in frame</li>
              <li className="flex items-center gap-2"><span className="text-red-400">&#10005;</span> Repeated integrity violations</li>
            </ul>
